import traceback
from get_car_price import get_car_features
...
def predict():
    if(model):
        try:
            ...
            fuel_types = ['Fuel_Type_CNG', 'Fuel_Type_Diesel',
...
@app.route("/predictWithJson", methods=["POST"])
def predictWithJson():
    if(model):
        try:
...
def getDetails():
    reqData = request.get_json()
    return jsonify(get_car_features("site:cardekho.com " + reqData['query']))
    # return jsonify({})
